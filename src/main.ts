...
`
  ...
  try {
    ...
    if (
      firstPostAttemptError.response?.data.message.includes('A page with this title already exists')
    ) {
      try {
        ...
        const response = await confluence.updateExistingPage(pageMetadataResponse.data, page)
        ...
        setFailed(`Error occurred attempting to update Confluence on page: ${page.title}`)
      }
    } else {
      error(
